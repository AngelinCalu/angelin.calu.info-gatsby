import React from 'react';

const Bio = () => {
    return (
        <section className="space-y-2">
            <h3 className="text-lg">
                Hi! I’m <em className="font-bold">Angelin Calu</em>
            </h3>
            <p>
                Born in Braila, Romania, currently living in Turku, Finland, father of two, entreprenour, passionate
                about technology in general, started coding years before Stackoverflow launched.
            </p>
            <p>
                Experienced in building extensive SaaS applications from designing (lately preferring Figma as a main
                tool) to writing both front and backend end logic (<b>Full-Stack</b>), testing, deployment, including
                basic server management tasks.
            </p>
        </section>
    );
};

export default Bio;
